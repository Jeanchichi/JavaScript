document.addEventListener('DOMContentLoaded', () => {
    ...
    for(let i=0 ; i < checkArray.length ; i++){
        ...
        let ind1 = checkArray[i][0]
        let ind2 = checkArray[i][1]
        let ind3 = checkArray[i][2]
        let ind4 = checkArray[i][3]

        if(squares[ind1].classList.contains('player-one') && squares[ind2].classList.contains('player-one') && 
        squares[ind3].classList.contains('player-one') && squares[ind4].classList.contains('player-one'))
        {
            alert("Player 1 Won!")
            result.innerHTML = "Player 1"
        }
         
        if(squares[ind1].classList.contains('player-two') && squares[ind2].classList.contains('player-two') && 
         squares[ind3].classList.contains('player-two') && squares[ind4].classList.contains('player-two'))
        {
            alert("Player 2 Won!")
    ...
    squares.forEach(squar => squar.addEventListener('click',checkBoard ))
